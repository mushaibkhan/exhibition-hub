...
import { Search, Eye, ExternalLink, Square, Pencil, Check, X, Plus } from 'lucide-react';
...
const Stalls = () => {
  const { 
    ...
    getPaymentsByTransactionId, updateStall, addStall, deleteStall
  } = useData();
  ...
  const [newStall, setNewStall] = useState({ stall_number: '', zone: 'Floor 1', base_rent: '' as string, notes: '' });
  const [addingStall, setAddingStall] = useState(false);

  const handleAddStall = async () => {
    if (!newStall.stall_number.trim()) {
      toast({ title: 'Stall number is required', variant: 'destructive' });
      return;
    }
    const rent = Number(newStall.base_rent);
    if (!Number.isFinite(rent) || rent < 0) {
      toast({ title: 'Please enter a valid base rent', variant: 'destructive' });
      return;
    }
    setAddingStall(true);
    try {
      await addStall({
        stall_number: newStall.stall_number.trim(),
        zone: newStall.zone,
        base_rent: rent,
        notes: newStall.notes.trim() || undefined,
      });
      toast({ title: 'Stall created', description: `Stall ${newStall.stall_number} has been added.` });
      setAddDialogOpen(false);
      setNewStall({ stall_number: '', zone: 'Floor 1', base_rent: '', notes: '' });
    } catch (error: any) {
      toast({ title: 'Failed to add stall', description: error?.message || 'Unknown error', variant: 'destructive' });
    } finally {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center justify-between">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            {isAdmin && (
              <Button onClick={() => setAddDialogOpen(true)} className="h-10 min-h-[44px]">
                <Plus className="h-4 w-4 mr-2" />
                Add Stall
              </Button>
            )}
            <Button variant="outline" onClick={() => navigate('/transactions')} className="flex-1 sm:flex-initial h-10 min-h-[44px]">
                  ...
                  return (
                    <TableRow key={stall.id}>
                      <TableCell 
                        ...
                        onClick={() => navigate('/floor-layout', { state: { stallId: stall.id } })}
                      >
                        {stall.stall_number}
                      </TableCell>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={e => setNewStall(prev => ({ ...prev, stall_number: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              <Label htmlFor="zone">Zone</Label>
              <Select value={newStall.zone} onValueChange={v => setNewStall(prev => ({ ...prev, zone: v }))}>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min={0}
                placeholder="0"
                value={newStall.base_rent}
                onChange={e => setNewStall(prev => ({ ...prev, base_rent: e.target.value }))}
              />
            </div>
            <div className="space-y-2">
              ...
              <Textarea
                ...
                value={newStall.notes}
                onChange={e => setNewStall(prev => ({ ...prev, notes: e.target.value }))}
              />
            </div>
          </div>
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setAddDialogOpen(false)}>Cancel</Button>
            <Button onClick={handleAddStall} disabled={addingStall}>
